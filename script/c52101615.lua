...
function c52101615.cfilter(c,tp)
	if c:IsControler(tp) or c:IsHasEffect(EFFECT_EXTRA_RELEASE) then return false end
	for i,pe in ipairs({c:IsHasEffect(EFFECT_EXTRA_RELEASE_NONSUM)}) do
		if pe:CheckCountLimit(tp) then return true end
	end
	return false
end
function c52101615.fcheck(tp,sg,fc)
	return sg:FilterCount(c52101615.cfilter,nil,tp)<=1
end
function c52101615.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		local mg=Duel.GetReleaseGroup(tp):Filter(Card.IsCanBeFusionMaterial,nil)
	...
	Auxiliary.FCheckAdditional=nil
	local exrc=mat:Filter(c52101615.cfilter,nil,tp):GetFirst()
	if exrc then
		local fg=Group.CreateGroup()
		for i,pe in ipairs({exrc:IsHasEffect(EFFECT_EXTRA_RELEASE_NONSUM)}) do
			if pe:CheckCountLimit(tp) then fg:AddCard(pe:GetHandler()) end
		end
		if fg:GetCount()>1 then fg=fg:Select(tp,1,1,nil) end
		local fc=fg:GetFirst()
		for i,pe in ipairs({exrc:IsHasEffect(EFFECT_EXTRA_RELEASE_NONSUM)}) do
			if pe:GetHandler()==fc then
