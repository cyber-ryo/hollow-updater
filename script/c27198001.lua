...
function c27198001.fselect(c,tp,rg,sg)
	...
	if sg:GetCount()<2 then
		...
		if Duel.GetMZoneCount(tp,sg)>0 then
			...
			res=Duel.CheckReleaseGroup(tp,nil,0,nil)
		end
	end
	sg:RemoveCard(c)
	return res
...
function c27198001.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		local sg=Duel.SelectReleaseGroup(tp,c27198001.fselect,2,2,g,tp,rg,g)
		g:Merge(sg)
	else
		while g:GetCount()<2 do
			local sg=Duel.SelectReleaseGroup(tp,c27198001.fselect,1,1,g,tp,rg,g)
			g:Merge(sg)
		end
	end
